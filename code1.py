...
def solve_c3(teams, results):
    ...
    for teamA, teamB, winner in results:
          # to skip the eliminated
        if teamA in eliminated or teamB in eliminated:
            continue

        loser = teamB if winner == teamA else teamA

        streak[winner] = 0  # Wining streak 
        streak[loser] += 1  #losing Streak
         
         # eliminate if streak equal to 2
        if streak[loser] == 2:
            eliminated.append(loser)
    survivors = [team for team in teams if team not in eliminated]
    survivors.sort()
...
print("Surviving Teams:", solve_c3(teams, results))
